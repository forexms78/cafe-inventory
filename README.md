...
### 운영 편의 기능
- **품목 검색**: 상단 검색창 → 카테고리 자동 전환 + 해당 행 하이라이트 스크롤
...
| **Action** | 3개 위치 재고 합산 자동 계산, 최소 수량 기준 위험/주의/정상 자동 분류, 부족 품목 클릭 시 쿠팡·네이버쇼핑 즉시 연결, 드래그앤드롭 정렬, 유통기한 캘린더 추적, 품목 검색, 오너/매니저 권한 분리를 Next.js + Supabase로 구현. 낙관적 업데이트 + API 실패 롤백으로 데이터 정합성 유지 |
| **Result** | 재고 파악 → 주문까지 클릭 2회로 단축, 전 카테고리 부족 현황 대시보드 1화면 제공, 실제 카페 현장 투입, 모바일 환경 완전 지원 |
