...
export function fireExplosion(
  ...
  rects.forEach(rect => {
    ...
    const aStep = Math.max(1, (aCols * aRows) / 280);
    for (let idx = 0; idx < aCols * aRows; idx += aStep) {
      ...
      particles.push({
        ...
        delay: rnd(0, 0.03), duration: rnd(2.0, 3.5),
        isCircle: false,
      });
    }
    ...
    const bStep = Math.max(1, (bCols * bRows) / 55);
    for (let idx = 0; idx < bCols * bRows; idx += bStep) {
      ...
      particles.push({
        ...
        delay: rnd(0, 0.05), duration: rnd(2.3, 4.0),
        isCircle: false,
      });
    }
    ...
    const cStep = Math.max(1, (cCols * cRows) / 12);
    for (let idx = 0; idx < cCols * cRows; idx += cStep) {
      ...
      particles.push({
        ...
        delay: rnd(0, 0.06), duration: rnd(2.8, 4.5),
        isCircle: false,
      });
    }
  });
  ...
  for (let i = 0; i < 180; i++) {
    ...
    particles.push({
      ...
      delay: rnd(0, 0.08), duration: rnd(1.2, 2.8),
  ...
  const animate = (now: number) => {
    ...
    // 플래시 (0~0.22s): 따뜻한 주황-흰색
    if (t < 0.22) {
      const ft = t / 0.22;
      const fa = 0.88 * (1 - ft * ft);
      ctx.fillStyle = `rgba(255, 220, 120, ${fa.toFixed(3)})`;
